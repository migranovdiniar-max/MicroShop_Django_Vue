...
pm.test("Comment created successfully", function () {
  ...
  pm.expect(jsonData.content).to.eql(
    "This is my first comment on this amazing blog post! Thank you for sharing such valuable information."
  );
...
pm.test("Comments include replies", function () {
  ...
  if (jsonData.comments.length > 0) {
    var mainComment = jsonData.comments.find(
...
pm.test("Posts pagination works with pinning", function () {
  ...
  if (jsonData.results.length > 0) {
    ...
    if (pinnedPosts.length > 0 && regularPosts.length > 0) {
      var lastPinnedIndex = jsonData.results.findLastIndex(
        (post) => post.is_pinned
      );
      var firstRegularIndex = jsonData.results.findIndex(
        (post) => !post.is_pinned
      );
...
if (pm.request.url.toString().includes(pm.environment.get("base_url"))) {
  pm.sendRequest(
    ...
    function (err, response) {
      if (err) {
        console.log(
          "⚠️  Django server may not be running on " +
