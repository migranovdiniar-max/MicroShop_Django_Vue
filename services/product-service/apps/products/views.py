...
def reserve_product(request, product_id):
    try:
        ...
        if product.reserve_quantity(quantity):
            return Response({
                "success": True,
                "message": f"{quantity} units of product {product.name} reserved successfully",
                "remaining_stock": f"{product.stock_quantity} units left"
            })
        else:
            return Response({
                "success": False,
                "message": f"Insufficient stock for product {product.name}",
                "available_stock": f"{product.stock_quantity} units available",
...
def release_product(request, product_id):
    try:
        ...
        return Response({
            "success": True,
            "message": f"{quantity} units of product {product.name} released successfully",
            "current_stock": f"{product.stock_quantity} units available"
        })
